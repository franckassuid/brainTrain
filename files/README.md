# Base de données — Application d'entraînement mental (v1 + v2 + v3)

Base SQLite locale contenant 450 parties prêtes à jouer : 50 grilles de
Sudoku, 50 parties de Mastermind, 50 Nonogrammes, 50 puzzles Hashi
(Ponts) et 250 niveaux du Compte est bon. Aucune interface n'est incluse
à ce stade : uniquement la base, les données, un script de génération et
...
> types malgré leurs effectifs très différents (voir plus bas).
>
> Dans toutes les versions, les jeux déjà présents en base (Sudoku et
> Mastermind depuis la v1) ne sont ni supprimés ni régénérés par le
> script — voir la section "Extensions" plus bas.

## Fichiers

| Fichier                       | Rôle                                                                 |
|--------------------------------|-----------------------------------------------------------------------|
| `schema.sql`                   | Définition des 5 tables (`sudoku_puzzles`, `mastermind_games`, `nonogram_puzzles`, `hashi_puzzles`, `compte_est_bon_puzzles`) |
...
| `generate_data.py`             | Script de remplissage : génère et insère les 450 parties (préserve Sudoku/Mastermind existants) |
| `query.py`                     | `get_games()` / `get_random_game()` — filtrage par type (5) / difficulté / durée, **tirage équilibré en 2 étapes** |
| `test_data.py`                 | Tests de validation (comptes, cohérence, unicité, règles de chaque jeu, équilibre du tirage) |
...
base (250 pour le Compte est bon contre 50 pour les 4 autres types). Un
...
450, il apparaîtrait environ 56 % du temps au lieu du 20 % attendu pour
5 types équiprobables.

**Solution** : `get_random_game()` (dans `query.py`) applique un tirage
en **deux étapes** lorsqu'aucun `game_type` n'est précisé :

1. on détermine, parmi les 5 types, ceux qui ont au moins un jeu
...
Si un type est demandé explicitement (`game_type="compte_est_bon"` par
...
type représente bien environ 20 % des résultats (± 6 %), y compris
lorsqu'un filtre de difficulté est appliqué.
...
```bash
# Générer (ou régénérer) la base et les 100 parties
...
# Un jeu au hasard, tous types confondus (5 types, tirage ÉQUILIBRÉ par type
...
## Prochaines étapes (hors périmètre de cette v1/v2/v3)

- Interface de jeu (Sudoku, Mastermind, Nonogramme, Hashi, Compte est bon)
